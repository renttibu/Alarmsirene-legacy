...
class Alarmsirene1 extends IPSModule # Variable
{
    // Helper
    use AS1_alarmProtocol;
    use AS1_alarmSiren;
    ...
    public function Create()
    {
        ...
        $this->RegisterVariableInteger('SignallingAmount', 'Auslösungen', '', 20);
        if ($id == false) {
        ...
        $this->RegisterVariableInteger('ResetSignallingAmount', 'Rückstellung', $profile, 30);
        $this->EnableAction('ResetSignallingAmount');
        // Mute mode
        $profile = 'AS1.' . $this->InstanceID . '.MuteMode.Reversed';
        if (!IPS_VariableProfileExists($profile)) {
        ...
        $this->RegisterVariableBoolean('MuteMode', 'Stummschaltung', $profile, 40);
    ...
    public function MessageSink($TimeStamp, $SenderID, $Message, $Data)
    {
        ...
        switch ($Message) {
            ...
            case VM_UPDATE:
                ...
                $scriptText = 'AS1_CheckTriggerVariable(' . $this->InstanceID . ', ' . $SenderID . ', ' . $valueChanged . ');';
