...
class AlarmsireneHMSecSirWM extends IPSModule
{
    // Helper
    use ASHMSECSIRWM_alarmProtocol;
    use ASHMSECSIRWM_alarmSiren;
    use ASHMSECSIRWM_backupRestore;
    use ASHMSECSIRWM_muteMode;
    use ASHMSECSIRWM_triggerVariable;
    ...
    public function Create()
    {
        ...
        // Status
        $profile = 'ASHMSECSIRWM.' . $this->InstanceID . '.Status';
        ...
        $profile = 'ASHMSECSIRWM.' . $this->InstanceID . '.ResetSignallingAmount';
        if (!IPS_VariableProfileExists($profile)) {
            IPS_CreateVariableProfile($profile, 1);
        }
        ...
        // Mute mode
        $profile = 'ASHMSECSIRWM.' . $this->InstanceID . '.MuteMode.Reversed';
        if (!IPS_VariableProfileExists($profile)) {
        ...
        // Timers
        $this->RegisterTimer('DeactivatePreAlarm', 0, 'ASHMSECSIRWM_DeactivatePreAlarm(' . $this->InstanceID . ');');
        $this->RegisterTimer('ExecuteMainAlarm', 0, 'ASHMSECSIRWM_ExecuteMainAlarm(' . $this->InstanceID . ', false);');
        $this->RegisterTimer('DeactivateAlarmSiren', 0, 'ASHMSECSIRWM_ToggleAlarmSiren(' . $this->InstanceID . ', false);');
        $this->RegisterTimer('ResetSignallingAmount', 0, 'ASHMSECSIRWM_ResetSignallingAmount(' . $this->InstanceID . ');');
        $this->RegisterTimer('StartMuteMode', 0, 'ASHMSECSIRWM_StartMuteMode(' . $this->InstanceID . ');');
        $this->RegisterTimer('StopMuteMode', 0, 'ASHMSECSIRWM_StopMuteMode(' . $this->InstanceID . ',);');
    }
    ...
    public function ApplyChanges()
    {
        ...
        $properties = ['AlarmSiren', 'AlarmProtocol'];
    ...
    public function Destroy()
    {
        ...
        if (!empty($profiles)) {
            foreach ($profiles as $profile) {
                $profileName = 'ASHMSECSIRWM.' . $this->InstanceID . '.' . $profile;
    ...
    public function MessageSink($TimeStamp, $SenderID, $Message, $Data)
    {
        ...
        switch ($Message) {
            ...
            case VM_UPDATE:
                ...
                $scriptText = 'ASHMSECSIRWM_CheckTriggerVariable(' . $this->InstanceID . ', ' . $SenderID . ', ' . $valueChanged . ');';
    ...
    public function GetConfigurationForm()
    {
        ...
        $formData['elements'][1]['items'][0] = [
            ...
            'items' => [$formData['elements'][1]['items'][0]['items'][0] = [
                'type'    => 'SelectInstance',
                'name'    => 'AlarmSiren',
                'caption' => 'HM-Sec-Sir-WM Instanz, Kanal 3',
                'width'   => '600px',
            ],
        ...
        $formData['status'][0] = [
            'code'    => 101,
            'icon'    => 'active',
            'caption' => 'Alarmsirene HM-Sec-Sir-WM wird erstellt',
        ];
        $formData['status'][1] = [
            'code'    => 102,
            'icon'    => 'active',
            'caption' => 'Alarmsirene HM-Sec-Sir-WM ist aktiv (ID ' . $this->InstanceID . ')',
        ];
        $formData['status'][2] = [
            'code'    => 103,
            'icon'    => 'active',
            'caption' => 'Alarmsirene HM-Sec-Sir-WM wird gelöscht (ID ' . $this->InstanceID . ')',
        ];
        $formData['status'][3] = [
            'code'    => 104,
            'icon'    => 'inactive',
            'caption' => 'Alarmsirene HM-Sec-Sir-WM ist inaktiv (ID ' . $this->InstanceID . ')',
        ];
        $formData['status'][4] = [
            'code'    => 200,
            'icon'    => 'inactive',
